 ...
 * libmsg.c
 *
 * Common message handling functions for Asgard bulletin board system.
 */
...
extern MessageBuffer  *msgBuf, *tempMess;
FILE   *msgfl, *msgfl2;
...
char getMessage(int (*Source)(void), char FromNet, char all, char ClearOthers)
{
    ...
    if (!FromNet) {      /* only do this for messages from our own msgs */
        ...
        sprintf(msgBuf->mbgrpNo, "%d",0);
        sprintf(msgBuf->mbgrpGen, "%d",0);
    ...
    do  {
        ...
        case '4':
            c = (*Source)();
            switch(c) {
            case 'X': getMsgStr(Source, msgBuf->mbmisc,  NAMESIZE);  break;
            case 'G': getMsgStr(Source, msgBuf->mbgrpNo, NAMESIZE);  break;
            case 'E': getMsgStr(Source, msgBuf->mbgrpGen,NAMESIZE);  break;
            case 'N': getMsgStr(Source, msgBuf->mbFidoNode, NAMESIZE); break;
            case 'S': getMsgStr(Source, msgBuf->mbSubj, sizeof msgBuf-> mbSubj);
                                                                      break;
            default:  unGetMsgChar(c);                               break;
            }
            break;
...
void getMsgStr(int (*Source)(void), char *dest, int lim)
{
    ...
    while ((c = (*Source)())!=0 && c != EOF) { /* read the complete string     */
 ...
 * getMsgChar()
 *
 * This function returns sequential chars from AsgMsg.Sys.
 */
int getMsgChar()
{
    long work;
    ...
    if (mFile1.thisChar == 0) {
        ...
        crypte((AN_UNSIGNED *) mFile1.sectBuf, MSG_SECT_SIZE, 0);
    }
    return(toReturn);
}
...
void startAt(FILE *whichmsg, struct mBuf *mFile, SECTOR_ID sect, int byt)
{
    ...
    crypte((AN_UNSIGNED *) mFile->sectBuf, MSG_SECT_SIZE, 0);
}
...
void InitMsgBase()
{
    ...
    makeSysName(name, "asgmsg.sys", &cfg.msgArea);
    openFile(name, &msgfl);

    if (cfg.BoolFlags.mirror) {
        makeSysName(name, "asgmsg.sys", &cfg.msg2Area);
...
void InitBuffers()
{
    static SListBase MsgBase = { NULL, (void *) ChkCC, NULL, free, (void *) strdup };
...
#if 0
    msgBuf->mbCC = MsgBase;
    msgBuf->mbOverride = MsgBase;
    msgBuf->mbInternal = MsgBase;
    msgBuf->mbForeign = MsgBase;
    tempMess->mbCC = MsgBase;
    tempMess->mbOverride = MsgBase;
    tempMess->mbInternal = MsgBase;
    tempMess->mbForeign = MsgBase;
#endif
}
...
char findMessage(SECTOR_ID loc, MSG_NUMBER id, char ClearOthers)
{
    ...
    do {
        getMessage(getMsgChar, FALSE, (noStop==LOADIT) ? TRUE : FALSE,
                   ClearOthers);
        here = atol(msgBuf->mbId);
    } while (here != id &&  mFile1.thisSector == loc);

    if (here != id) printf("Ooops, couldn't find %ld @ %d.\n ", id, loc);
    return ((here == id));
}
